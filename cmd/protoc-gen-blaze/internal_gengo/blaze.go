...
var (
	...
	jsonpbPackage       goImportPath = protogen.GoImportPath("google.golang.org/protobuf/encoding/protojson")
...
func (s *Blaze) generateClient(name string, g *protogen.GeneratedFile, file *fileInfo, service *protogen.Service) {
	...
	if name == "JSON" {
		...
		g.P(`var buf []byte`)
		g.P(`marshaler := &`, g.QualifiedGoIdent(jsonpbPackage.Ident("MarshalOptions")), `{UseProtoNames: true}`)
		...
		g.P(`return `, g.QualifiedGoIdent(blazePackage.Ident("ErrorInternalWith")), `(err, "failed to read response body")`)
		g.P(`}`)
		g.P(`  unmarshaler := `, g.QualifiedGoIdent(jsonpbPackage.Ident("UnmarshalOptions")), `{DiscardUnknown: true}`)
...
func (s *Blaze) generateServerJSONMethod(g *protogen.GeneratedFile, service *protogen.Service, method *protogen.Method) {
	...
	g.P()
	g.P(`  var buf []byte`)
	g.P(`  marshaler := &`, g.QualifiedGoIdent(jsonpbPackage.Ident("MarshalOptions")), `{`)
...
func (s *Blaze) generateServerProtobufMethod(g *protogen.GeneratedFile, service *protogen.Service, method *protogen.Method) {
	...
	g.P(`  }`)
	g.P(`  reqContent := new(`, g.QualifiedGoIdent(method.Input.GoIdent), `)`)
	g.P(`  if err = `, g.QualifiedGoIdent(jsonpbPackage.Ident("Unmarshal")), `(buf, reqContent); err != nil {`)
	...
	g.P(`  }`)
	g.P()
	g.P(`  respBytes, err := `, g.QualifiedGoIdent(jsonpbPackage.Ident("Marshal")), `(respContent)`)
	g.P(`  if err != nil {`)
